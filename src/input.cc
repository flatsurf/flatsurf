...
/*
    ...
    ratpoly cyc = cyclotomic_poly(n);
    vector<bigrat> coeffs = cyc.coefficients;
    (NumberField<bigrat>::F) = new NumberField(&coeffs[0], cyc.degree(), nth_rootofunity);
...
void TwoComplex::BuildTriangle(int alpha, int beta, int gamma)
{
    ...
    algebraicQ zeta = algebraicQ(1, NumberField<bigrat>::F);  /* zeta = exp(2*pi*i/denom) */

    algebraicQ one_F = algebraicQ(0,NumberField<bigrat>::F); /* this is 1 in F */
    cout << "zeta = " << zeta << " one_F = " << one_F << endl;
    cout << "zeta.cx = " << zeta.tocomplex() << " one_F.cx = " << one_F.tocomplex() << endl;

    NumberField<bigrat>::F->print_mt();


    algebraicQ q_alg = (one_F-zeta.pow(alpha+gamma))/(one_F-zeta.pow(gamma));
    algebraicQ p_alg = one_F;

    algebraicQ num = one_F-zeta.pow(alpha+gamma);
    
    cout << "gamma = " << gamma << " zeta.pow(gamma) = " << zeta.pow(gamma) << endl;
...
//          -2*abs(p)*abs(q)*cos(a)-abs(q-p)*abs(q-p) << endl; 
    ...
    VertexPtr v1 = AddVertex(UNDEFINED);
    VertexPtr v2 = AddVertex(UNDEFINED);
...
/* Calculate the coordinates of a quadrilateral with angles 
  adds a quadrilateral with angles  2*pi*a/(a+b+c+d), 2*pi*b/(a+b+c+d), 2*pi*c/(a+b+c+d), 2*pi*d/(a+b+c+d), where a,b,c,d are the entries of int_angles. The
result is a quadrilateral with vertices 0, p, q, and r. The angles at these 
vertices are in this order.  
The biggest angle is assumed to be  2*pi*a/(a+b+c+d), which is the angle at 0. 
The angle at a is bisected into a1 and a2, using the formula below. Also, p is
always going to be (1,0). 
...
//			      zeta.pow(alpha) + r1_alg*zeta.pow(alpha+delta)}; 
    ...
    VertexPtr v1 = AddVertex(UNDEFINED);
    VertexPtr v2 = AddVertex(UNDEFINED);
    VertexPtr v3 = AddVertex(UNDEFINED);
...
VertexPtr TwoComplex::GetVertex(int id)
{
...
//  printf("p_vec_re = %g, p_vec_im = %g\n", *p_vec_re, *p_vec_im); 
  ...
  /*
#ifdef USE_LONG_DOUBLE
    if( fscanf(fp,"E%d ( V%d V%d F%d F%d ) (%Lg,%Lg)", &ue_id, &v0_id, &v1_id, &f0_id, &f1_id, &vec_re, &vec_im) != 7 ) {
#else
    if( fscanf(fp,"E%d ( V%d V%d F%d F%d ) (%lg,%lg)", &ue_id, &v0_id, &v1_id, &f0_id, &f1_id, &vec_re, &vec_im) != 7 ) {
#endif
	ERR_RET2("bad edge line; line =", line_number);
    }
...
void TwoComplex::read_edge_line(FILE *fp,int line_number)
{
    ...
    *s = '\0';
    s++;
    if( sscanf(buf,"E%d ( V%d V%d F%d F%d )", &ue_id, &v0_id, &v1_id, &f0_id, &f1_id) != 5 ) {
...
/*
 void TwoComplex::BuildTriangleAlgebraic()
{
  COORD a=  5 * MY_PI/18;
  COORD b = MY_PI/3;
    ...
    bigrat poly[] = {1, 0, 0, -1, 0, 0, 1};
    complex<double> rootofunity(::cos(MY_PI/9), 
				  ::sin(MY_PI/9));
    (S->field) = new NumberField(poly, 6, rootofunity);
    S->field->store_conjugate(algebraic(1, S->field).pow(17));
...
//    std::cout << "check PI: " << sinl(MY_PI) << endl; 
    ...
    bigrat ptemp[6] = {bigrat(-2,3), bigrat(2,1), bigrat(-2,1), bigrat(4,3), bigrat(-2,1), 0};
    bigrat qtemp[6] = {0, bigrat(2, 1), bigrat(-2,1), bigrat(2, 1), bigrat(-2, 1), 0};
    algebraic p_alg(ptemp, S->field), 
      q_alg(qtemp, S->field);
...
//          -2*abs(p)*abs(q)*cos(a)-abs(q-p)*abs(q-p) << endl; 
    ...
    ue = AddUEdge(UNDEFINED,v0,v1,p, p_alg);
    oe = new OEdge(ue,1);
    ue->label = 1;
    tmp_oedge_list.insert(tmp_oedge_list.end(),*oe);

    ue = AddUEdge(UNDEFINED,v1,v2,q-p, q_alg-p_alg);
    oe = new OEdge(ue,1);
    ue->label = 2;
    tmp_oedge_list.insert(tmp_oedge_list.end(),*oe);


    ue = AddUEdge(UNDEFINED,v2,v0,-q, -q_alg);
    oe = new OEdge(ue,1);
    ue->label = 3;
    tmp_oedge_list.insert(tmp_oedge_list.end(),*oe);
