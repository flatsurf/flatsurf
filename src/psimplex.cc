...
{
    throw std::logic_error("roman_numeral: The implementation of this function had to be removed since we're not sure about  its license");
}
...
my_ostream::my_ostream(const string& filename) 
{
	...
	int count = 1; 
	for( VertexPtrIter i = S->vertices.begin(); i !=
		 S->vertices.end(); ++i ) {
	    int v_id = (*i)->id();
	...
	for(int j=0; j < 10; j++ ) {
	    tex_stream << "\\def\\c" << roman_numeral(j) << "color{[red!" << 10*(j+1) << "!white]}" << endl;
	}
    }
}


ofstream& my_ostream::tri()
{
...
PSimplex::PSimplex(Point p0, int i): p(p0)
{
    in_pcomplex = i;
}
...
PVertex::PVertex(VertexPtr v0, Point p0, int i) : PSimplex(p0,i)
{
...
SimplexPtr PVertex::sp()
{
    return static_cast<Simplex*>(s);
}
...
PUEdge::PUEdge(UEdgePtr e0, Point p0, int i) : PSimplex(p0,i)
{
...
SimplexPtr PUEdge::sp()
{
    return static_cast<Simplex*>(s);
}

PFace::PFace(FacePtr f0, Point p0, int i) : PSimplex(p0,i)
{
...
void PVertex::Draw(my_ostream& output_stream, COORD d)
{
	...
	output_stream.tex() << "\\filldraw \\v"<<roman_numeral(s->id())<<"color ("
			    << p.real() << "," << p.imag()
			    << ") circle (0.2cm); " << endl;
    }
...
void PFace::Draw(my_ostream& output_stream,COORD scale_factor)
{
    ...
    // q *= 0.05*scale_factor;
    // q += p1;
    // output_stream << "N" << s->id() <<" "<< p1.real() << " " <<p1.imag() <<" "<< q.real()<< " " <<q.imag() << "\n"; 

    /* draw extra segments (for displaying saddle connections ) */
    for(SegmentListIter i = s->segments_to_draw.begin(); i!=s->segments_to_draw.end(); ++i) {
	Point q1 = p+(*i).head*scale_factor;
	Point q2 = p+(*i).tail*scale_factor;
	output_stream.tri() << "N" << s->id() <<" "<< q1.real() << " " <<q1.imag() <<" "<< q2.real()<< " " <<q2.imag() << "\n";
	if ( tikz_output ) {
	    output_stream.tex() << "%N" << " cyl = " << i->cyl_on_left 
...
bool point_in_segment(Point p, Point q, Point r)
{
    ...
    if(Dist2(p,q) < THRESHOLD*THRESHOLD || Dist2(p, r) < THRESHOLD*THRESHOLD ) {
	return true;
    }
...
void PFace::DrawCylinders(my_ostream& output_stream,COORD scale_factor)
{
	...
	for( PointListIter m = (*j).verts.begin(); m!= (*j).verts.end(); ++m) {
		...
		PointListIter k_tmp = k1; k1 = k2; k2 = k_tmp;
		Point q_tmp = q1; q1 = q2; q2 = q_tmp;
	    }
	}
...
//	    fprintf(out_f,"...Colinear");
	    ...
	    COORD e_end = Dot((*k).headOffset<Point>()+(*k).vec_cx(), 
			       pv)/abs(pv); 
...
//    fprintf(out_f,"\n");
	...
	if( this->intersects((*k).headOffset<Point>()+offset,
		       (*k).vec_cx()) ) {
...
bool Face::contains( Point p) 
{
    ...
    /* hack */
    Point far_out_vector = 1000.0*(this->perimeter())*
	                      Point(0.34344, 0.565667);


    int count = 0;

    for(OEdgeIter k = oedges.begin(); k!= oedges.end(); ++k ) {
	if ( intersect_segment_interior(p, far_out_vector, 
			       (*k).headOffset<Point>(),(*k).vec_cx(),
			       intersection)) {
	    count++;
	}
...
bool TwoComplex::can_merge( UEdgePtr ue)
{
    ...
    Point offset_ = pf0->p + oe0->headOffset<Point>() - 
	pf1->p - oe1->tailOffset<Point>();
...
bool is_longer(UEdgePtr a, UEdgePtr b) 
{
...
#define DIM 16.0
	...
	if ( active && (!had_pcomplex) ) {
	    ...
	    relocate(pc, Point(global_max_x - min_x +SPACING*(max_x-min_x)
			       ,0.0));
//	    relocate(pc, Point(global_max_x - min_x +SPACING*(max_x-min_x)
//			       ,global_max_y - min_y +SPACING*(max_y-min_y)));
