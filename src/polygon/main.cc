...
#define MAX_SLICES 1
  ...
  while (1) {
    ...
    static struct option long_options[] = {
        ...
        {"libpolygon", no_argument, nullptr, '%'},
  ...
  if (libpolygon) {
    ...
    do {
      ...
      // TODO: This does probably not include the sector edges; should it
      // include the initial edge?
      for (auto saddle_connection :
           SaddleConnections(flat_triangulation,
                             Bound(static_cast<long long>(ceil(depth * depth))),
                             sectorBegin)) {
        // TODO: introduce a notion of marked vertices somewhere instead
        ...
        for (const HalfEdge e : flat_triangulation.edges()) {
          ...
          for (auto it = saddle_connections_in_same_direction.begin();
            ...
            // TODO: <FollowDir> (searches for vertices that are reachable from
            // "vertex" going exactly in the direction that "sc" is.
...
sc.add_saddle(tmp_start, tmp_end, tmp_algt);
                      ...
                      I don't really understand what this means, but I tried to
reproduce it here:
              */
