...
class NumberField {
 ...
 private:
  ...
  complex<COORD> embedding;  // numerical value for generator of field.  Used by
                             // algebraic<T>::to_complex().
...
inline algebraic<bigrat> to_rational(
    const algebraic<int64_t> &p)  // only works with given F
{
